...
def _get_conn():
    return psycopg2.connect(
        "postgresql://postgres:Atimos%4021AI04.26@db.kniksrpsxkwyrnchywyb.supabase.co:5432/postgres",
        sslmode="require"
    )
...
def generate():
    ...
    try:
        ...
        if image_mode:
            image_url = generate_image_url(improved_prompt)
            ai_text = image_url
        else:
    ...
    with db() as cur:
        cur.execute(
            "INSERT INTO generations (email, input, output, ts) VALUES (%s, %s, %s, %s)",
